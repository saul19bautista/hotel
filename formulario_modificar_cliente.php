...
if ($result->num_rows == 1) {
    ...
    $apellidoPaterno = isset($row['apellido_paterno']) ? $row['apellido_paterno'] : '';
    $apellidoMaterno = isset($row['apellido_materno']) ? $row['apellido_materno'] : '';
    $telefono = isset($row['telefono']) ? $row['telefono'] : '';
    $correoElectronico = isset($row['correo_electronico']) ? $row['correo_electronico'] : '';
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['modificar'])) {
    ...
    $apellidoPaternoNuevo = isset($_POST["apellidoPaterno"]) ? $_POST["apellidoPaterno"] : '';
    $apellidoMaternoNuevo = isset($_POST["apellidoMaterno"]) ? $_POST["apellidoMaterno"] : '';
    $telefonoNuevo = isset($_POST["telefono"]) ? $_POST["telefono"] : '';
    $correoElectronicoNuevo = isset($_POST["correoElectronico"]) ? $_POST["correoElectronico"] : '';

    // Actualizar los datos del cliente en la base de datos
    $sql = "UPDATE Clientes SET nombre = '$nombreNuevo', apellido_paterno = '$apellidoPaternoNuevo', apellido_materno = '$apellidoMaternoNuevo', telefono = '$telefonoNuevo', correo_electronico = '$correoElectronicoNuevo' WHERE id = $id";
