...
<head>
    ...
    <title>Registros de Check-out</title>
    <style>
        table {
...
<body>
    ...
    <table>
        ...
        // Consultar registros de check-out
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $busqueda = $_POST['busqueda'];
            $sql = "SELECT * FROM Checkout WHERE nombre LIKE '%$busqueda%' OR apellido_paterno LIKE '%$busqueda%' OR apellido_materno LIKE '%$busqueda%'";
        } else {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>" . $row["id"] . "</td><td>" . $row["nombre"] . "</td><td>" . $row["apellido_paterno"] . "</td><td>" . $row["apellido_materno"] . "</td><td>" . $row["fecha_checkout"] . "</td><td>" . $row["hora_checkout"] . "</td>";
                echo "<td>
                        <form method='post' action='modificar_checkout.php' style='display:inline;'>
                            <input type='hidden' name='id' value='" . $row['id'] . "'>
                            <button type='submit' class='btn-modificar'>Modificar</button>
                        </form>
                        <form method='post' action='eliminar_checkout.php' style='display:inline;'>
                            <input type='hidden' name='id' value='" . $row['id'] . "'>
            ...
            echo "<tr><td colspan='7'>No hay registros de check-out</td></tr>";
        }
        ?>
    </table>
    <?php
